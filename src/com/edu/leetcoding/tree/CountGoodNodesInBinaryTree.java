...
public class CountGoodNodesInBinaryTree {
    ...
    private void goodNodes(TreeNode parent, TreeNode current) {
        if(current == null) {
            return;
        }
        if(parent == null || parent.val <= current.val) {
            good++;
            goodNodes(current, current.left);
            goodNodes(current, current.right);
        } else {
            goodNodes(parent, current.left);
            goodNodes(parent, current.right);
        }
    }
}
