 ...
 *  This means that revisions 1 and 001 are considered equal. If a version number does not specify a revision at
 *  an index, then treat the revision as 0. For example, version 1.0 is less than version 1.1 because their revision 0s
...
public class CompareVersionNumbers {
    ...
    public int compareVersion(String version1, String version2) {
        ...
        int maxLength = Math.max(versionOneArray.length, versionTwoArray.length);//4
        int versionOne = 0;
        int versionTwo = 0;
        for (int i = 0; i < maxLength; i++) {
            if (i < versionOneArray.length) {
                versionOne = Integer.valueOf(versionOneArray[i]);
            } else {
                versionOne = 0;
            }
            if (i < versionTwoArray.length) {
                versionTwo = Integer.valueOf(versionTwoArray[i]);
            } else {
                versionTwo = 0;
            }
