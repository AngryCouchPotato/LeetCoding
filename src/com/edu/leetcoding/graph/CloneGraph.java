...
public class CloneGraph {
    ...
    public Node cloneGraph(Node node) {
        ...
        while(queue.size() > 0) {
            ...
            Node cachedNode = cache.get(currentNode.val);
            for(Node neighbor : currentNode.neighbors) {
                Node clonedNode = cache.get(neighbor.val);
                if(clonedNode == null) {
                    clonedNode = new Node(neighbor.val);
                    cache.put(neighbor.val, clonedNode);
                    queue.add(neighbor);
                }
                cachedNode.neighbors.add(clonedNode);
            }
        }
